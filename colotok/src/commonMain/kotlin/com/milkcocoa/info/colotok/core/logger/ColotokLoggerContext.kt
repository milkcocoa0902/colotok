...
class ColotokLoggerContext {
    ...
    fun withAttrs(attrs: Map<String, String>) =
        apply {
            ensureNotFrozen()
            this.attrs.clear()
     ...
     */
    fun withMetrics(collector: MetricsCollector) =
        apply {
            ensureNotFrozen()
    ...
    fun addProvider(provider: Provider) =
        apply {
            ...
            val baseCollector =
                when (val spec = provider.config.metricsSpec) {
            ...
            if (provider.config.enableInternalMetricsLogging) {
                ...
                if (baseCollector == NoOpMetricsCollector) {
                    ...
                    provider.effectiveMetricsCollector =
                        CompositeMetricsCollector(listOf(baseCollector, internalCollector))
                }
            } else {
    ...
    companion object {
        ...
        private var _default: ColotokLoggerContext =
        ...
        public fun setDefault(logger: ColotokLoggerContext) {
            _default = logger
        }
    }
}