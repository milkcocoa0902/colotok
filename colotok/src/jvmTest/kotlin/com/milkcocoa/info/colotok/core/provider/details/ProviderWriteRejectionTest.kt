...
class ProviderWriteRejectionTest {
    ...
    private class RecordingMetricsCollector : MetricsCollector {
        ...
        override fun updateBufferSize(providerName: String, size: Int) = Unit
    ...
    @Test
    fun default_suspend_queue_accepts_fifo_prefix_and_rejects_newest_record() = runBlocking {
    ...
    @Test
    fun terminal_rejections_are_classified_by_close_cancel_and_failure() = runBlocking {
        ...
        val failedProvider = object : Provider(ConsoleProviderConfig()) {
            override suspend fun onMessage(record: LogRecord) {
    ...
    @Test
    fun failed_channel_close_cause_is_classified_as_provider_failed() = runBlocking {
        val firstEntered = CompletableDeferred<Unit>()
        val releaseFirst = CompletableDeferred<Unit>()
    ...
    @Test
    fun filtered_and_metrics_records_do_not_report_rejection_errors() = runBlocking {
        val collector = RecordingMetricsCollector()
    ...
    @Test
    fun rejection_metric_collector_failure_is_isolated() = runBlocking {
        val provider = recordingProvider().also {
    ...
    ): Provider = object : Provider(config) {
