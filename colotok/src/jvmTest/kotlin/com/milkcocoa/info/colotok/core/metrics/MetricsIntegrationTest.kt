...
class MetricsIntegrationTest {
    ...
    fun testMetricsInheritance() {
        ...
        assertTrue(
            collector.logCounts.containsKey(com.milkcocoa.info.colotok.core.level.LogLevel.INFO to "StubProvider")
        )
    }
    ...
    @Test
    fun testInternalLoggingMetrics() =
        runBlocking {
    ...
    @Test
    fun testMetricsInternalAndInherit() =
        runBlocking {
            val contextCollector = TestMetricsCollector()
            val receivedRecords = mutableListOf<LogRecord>()
            ...
            assertEquals(
                1,
                contextCollector.logCounts.values.sum(),
                "Inherited collector should be called even with internal logging"
            )

            // Should also have the metrics log record
            assertTrue(
                receivedRecords.any {
                    it is LogRecord.Metrics && it.msg.contains("LogCount increased")
                },
                "Internal logging should be performed"
            )
        }

    @Test
    fun internal_logging_of_buffer_rejection_is_finite_and_non_recursive() =
        runBlocking {
            val collector = TestMetricsCollector()
    ...
    @Test
    fun collector_failure_does_not_fail_write_flush_or_join() =
        runBlocking {
            val provider =
