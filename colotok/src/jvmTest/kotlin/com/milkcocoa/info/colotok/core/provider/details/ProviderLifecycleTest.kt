...
class ProviderLifecycleTest {
    ...
    @Test
    fun flush_waits_for_accepted_records_and_calls_onFlush() =
        runBlocking {
    ...
    @Test
    fun flush_after_close_throws_provider_closed_exception() =
        runBlocking {
            val provider = RecordingProvider()
    ...
    @Test
    fun flush_after_force_throws_provider_closed_exception() =
        runBlocking {
            val provider = RecordingProvider()
    ...
    @Test
    fun join_after_force_does_not_report_graceful_success() =
        runBlocking {
            val provider = RecordingProvider()
    ...
    @Test
    fun on_flush_failure_completes_pin_exceptionally() =
        runBlocking {
    ...
    @Test
    fun on_message_failure_is_rethrown_by_flush_and_join() =
        runBlocking {
    ...
    @Test
    fun on_closed_runs_once_for_repeated_shutdown_calls() =
        runBlocking {
            val provider = RecordingProvider()
    ...
    @Test
    fun flush_excludes_records_accepted_after_its_marker() =
        runBlocking {
    ...
    @Test
    fun concurrent_flushes_complete_independently() =
        runBlocking {
            val provider = RecordingProvider()
    ...
    @Test
    fun flush_timeout_includes_waiting_to_enqueue_marker() =
        runBlocking {
    ...
    @Test
    fun flush_marker_linearized_before_close_completes_successfully() =
        runBlocking {
            val flushEntered = CompletableDeferred<Unit>()
            val releaseFlush = CompletableDeferred<Unit>()
            val provider =
    ...
    @Test
    fun write_before_close_is_drained_and_write_after_close_is_rejected() =
        runBlocking {
            val provider = RecordingProvider()
    ...
    @Test
    fun on_closed_failure_is_preserved_for_graceful_and_force_shutdown() =
        runBlocking {
    ...
    @Test
    fun graceful_final_flush_failure_makes_join_fail_with_original_cause() =
        runBlocking {
