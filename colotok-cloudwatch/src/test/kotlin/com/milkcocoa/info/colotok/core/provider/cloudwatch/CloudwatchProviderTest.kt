...
class CloudwatchProviderTest {
    ...
    @Test
    fun `unused provider does not initialize client during graceful shutdown`() = runTest {
        val factory = RecordingFactory()
    ...
    @Test
    fun `publish uses event time and stable chronological order`() = runTest {
        val factory = RecordingFactory()
        val provider = CloudwatchProvider(validConfig(factory))
    ...
    @Test
    fun `a failed later batch remains visible and stops subsequent publishing`() = runTest {
    ...
    @Test
    fun `partial batch failure retains the original records and may duplicate an earlier batch`() = runTest {
        val factory = RecordingFactory().apply { client.failOnPut = 2 }
        val provider = CloudwatchProvider(validConfig(factory))
    ...
    @Test
    fun `provisioning failure is propagated and used client closes once gracefully`() = runTest {
    ...
    private fun plain(message: String, timestamp: Instant) = LogRecord.PlainText(
        name = "test",
        msg = message,
        level = LogLevel.INFO,
        attr = emptyMap(),
    ...
    private class RecordingClient : CloudwatchClient {
        ...
        override suspend fun ensureLogStream(logGroup: String, logStream: String) = Unit
