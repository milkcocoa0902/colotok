...
class AsyncProviderTest {
    ...
    private fun record(index: Int): LogRecord = LogRecord.PlainText("test", "message $index", LogLevel.INFO, emptyMap())

    @Test
    fun graceful_close_publishes_sub_threshold_buffer_fifo_exactly_once_and_clears_it() =
        runTest {
            val metrics = RecordingMetricsCollector()
            val provider =
                LifecycleAsyncProvider(
    ...
    @Test
    fun final_publish_failure_is_rethrown_by_join_and_retains_the_attempted_batch() =
        runTest {
    ...
    @Test
    fun force_shutdown_does_not_publish_remaining_buffer_and_closes_once() =
        runTest {
    ...
    @Test
    fun failedPublishRecordsAreRetainedForNextSendTrigger() =
        runTest {
    ...
    @Test
    fun failed_backlog_retries_only_at_threshold_multiples() =
        runTest {
    ...
    @Test
    fun failed_backlog_can_expand_to_four_times_buffer_size_and_keeps_fifo() =
        runTest {
            val config = TestAsyncProviderConfig().apply { bufferSize = 2 }
    ...
    @Test
    fun newest_record_is_dropped_after_retention_capacity() =
        runTest {
            val config = TestAsyncProviderConfig().apply { bufferSize = 1 }
            val metrics = RecordingMetricsCollector()
            val provider =
                ControlledAsyncProvider(config).apply {
                    effectiveMetricsCollector = metrics
                }.track()
    ...
    @Test
    fun retention_capacity_is_capped_at_4096_records() =
        runTest {
    ...
    @Test
    fun manual_flush_failure_retains_batch_and_throws() =
        runTest {
    ...
    @Test
    fun metrics_record_does_not_generate_metrics() =
        runTest {
            val metrics = RecordingMetricsCollector()
            val provider =
                TestAsyncProvider(TestAsyncProviderConfig()).apply {
                    effectiveMetricsCollector = metrics
                }.track()
    ...
    @Test
    fun metrics_record_publish_failure_does_not_generate_error_metric() =
        runTest {
            val metrics = RecordingMetricsCollector()
            val provider =
                ControlledAsyncProvider(TestAsyncProviderConfig()).apply {
                    effectiveMetricsCollector = metrics
                }.track()
    ...
    @Test
    fun mixed_batch_publish_failure_reports_normal_record_metrics() =
        runTest {
            val metrics = RecordingMetricsCollector()
            val provider =
                ControlledAsyncProvider(
    ...
    @Test
    fun mixed_batch_publish_success_reports_cleared_buffer() =
        runTest {
            val metrics = RecordingMetricsCollector()
            val provider =
                ControlledAsyncProvider(
    ...
    @Test
    fun dropped_metrics_record_does_not_report_retention_error() =
        runTest {
            val metrics = RecordingMetricsCollector()
            val provider =
                ControlledAsyncProvider(TestAsyncProviderConfig()).apply {
    ...
    @Test
    fun collector_failure_does_not_break_async_worker_or_flush() =
        runTest {
            val provider =
                TestAsyncProvider(TestAsyncProviderConfig()).apply {
    ...
    @Test
    fun write_async_reports_acceptance_metrics_without_metrics_recursion() =
        runTest {
            val metrics = RecordingMetricsCollector()
            val provider =
                TestAsyncProvider(TestAsyncProviderConfig().apply { bufferSize = 2 }).apply {
                    effectiveMetricsCollector = metrics
                }.track()
    ...
    @Test
    fun write_async_after_close_is_rejected_and_reported() =
        runTest {
            val metrics = RecordingMetricsCollector()
            val provider =
                TestAsyncProvider(TestAsyncProviderConfig()).apply {
    ...
    @Test
    fun write_async_after_failed_channel_reports_failure_and_rethrows_it() =
        runTest {
            val metrics = RecordingMetricsCollector()
    ...
    @Test
    fun rejected_metrics_record_and_throwing_collector_do_not_change_terminal_behavior() =
        runTest {
            val metrics = RecordingMetricsCollector()
            val metricsProvider =
                TestAsyncProvider(TestAsyncProviderConfig()).apply {
